...
GROUP BY
    co.pizza_id -- How many Vegetarian and Meatlovers were ordered by each customer?
SELECT
    co.customer_id,
    SUM(
        CASE
...
SELECT
    MAX(wtf.num_pizza) AS total_pizza
FROM
    (
        SELECT
            order_id,
...
GROUP BY
    co.customer_id
ORDER BY
    co.customer_id -- What was the difference between the longest and shortest delivery times for all orders?
...
FROM
    clearned_ro ro
WHERE
    ro.duration != '' -- What was the average speed for each runner for each delivery and do you notice any trend for these values?
SELECT
    ro.runner_id,
    AVG(
...
GROUP BY
    ro.runner_id
ORDER BY
    ro.runner_id -- What is the successful delivery percentage for each runner?
SELECT
    ro.runner_id,
...
FROM
    (
        SELECT
            UNNEST(STRING_TO_ARRAY(extras, ', ')) :: int AS ingredients_id
        FROM
            clearned_co
    ) AS ingredients
GROUP BY
...
FROM
    (
        SELECT
            UNNEST(STRING_TO_ARRAY(exclusions, ', ')) :: int AS ingredients_id
        FROM
            clearned_co
    ) AS ingredients
GROUP BY
...
INGREDIENT_TOTAL AS (
    ...
    FROM
        ORDERS_WITH_EXTRAS_AND_EXCLUSIONS o
...
FROM
    clearned_ro ro
    INNER JOIN clearned_co co ON ro.order_id = co.order_id
    AND ro.cancellation = '';
...
WITH extras_expanded AS (
    ...
    FROM
        ...
        LEFT JOIN LATERAL (
            SELECT
                CAST(unnest(string_to_array(co.extras, ', ')) AS INT) AS extra_id
        ) e ON TRUE
)
SELECT
    SUM(
        CASE
            ...
            ELSE 10
        END + COALESCE(extra_count, 0) * 1 -- mỗi extra $1
    ) AS total_revenue
FROM
    (
        SELECT
...
-- Subquery
-- B1: lập danh sách tất cả các đơn hàng gồm loại pizza và extra của chúng
-- B2: tạo danh sách đơn hàng đếm các extra trong mỗi hàng
-- B3: tính tổng tiền dựa trên loại pizza và số lượng extra có trong mỗi pizza trong mỗi đơn hàng đó
SELECT
    SUM(
        CASE
            WHEN wow.pizza_id = 1 THEN 12
            ELSE 10
        END + COALESCE(wow.extra_count, 0) * 1
    ) AS total_revenue
FROM
    (
        SELECT
            ee.order_id,
            ee.pizza_id,
            COUNT(ee.extra_id) AS extra_count
        FROM
            (
                SELECT
                    co.order_id,
                ...
                FROM
                    ...
                    LEFT JOIN LATERAL (
                        SELECT
                            UNNEST(STRING_TO_ARRAY(co.extras, ', ')) AS extra_id
                    ) e ON TRUE
                    JOIN clearned_ro ro ON ro.order_id = co.order_id
                    AND ro.cancellation = ''
            ) ee
        GROUP BY
            ee.order_id,
...
CREATE TABLE IF NOT EXISTS `rating` (
    order_id INT,
    runnder_id INT,
    rate_score INT CHECK (
        rate_score BETWEEN 1
        AND 5
    ),
    -- rate_score INT CHECK (rate_score IN (1,2,3,4, 5)),
    PRIMARY KEY (order_id, runnder_id)
);
...
SELECT
    ...
    ro.runner_id,
    CASE
        WHEN r.rate_score IS NULL THEN 1
        ELSE r.rate_score
    END,
    co.order_time,
    ro.pickup_time,
    ...
    DATE(ro.pickup_time),
    ro.duration,
    AVG(ro.distance :: FLOAT / ro.duration :: FLOAT) AS avg_speed,
    COUNT(ro.order_id) total_pizza_delivery
FROM
    clearned_ro ro
    INNER JOIN clearned_co co ON co.order_id = ro.order_id
    AND ro.cancellation = ''
    INNER JOIN rating r ON r.order_id = ro.order_id
GROUP BY
    co.customer_id,
    ro.order_id,
    ro.runner_id,
...
SELECT
    SUM(
        CASE
            WHEN hehe.pizza_id = 1 THEN 12
            ELSE 10
        END - COALESCE(hehe.distance_paid_money, 0)
    ) AS total_profit
FROM
    (
        SELECT
            co.order_id,
            co.pizza_id,
            (ro.distance :: NUMERIC * 0.30) AS distance_paid_money
        FROM
            clearned_co co
            JOIN clearned_ro ro ON co.order_id = ro.order_id
            AND ro.cancellation = ''
    ) AS hehe;